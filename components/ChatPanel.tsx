  ...
  const panel = (
    <div className="flex flex-col h-full bg-white border-l border-gray-200">
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200">
        <span className="font-semibold text-sm text-gray-700">Coach</span>
        {!isDesktop && (
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.map((m, i) => (
          <div key={i} className={`text-sm ${m.role === 'user' ? 'text-right' : ''}`}>
            <span className={`inline-block rounded-lg px-3 py-2 max-w-xs text-left ${
              m.role === 'user'
        ...
        {loading && (
          <div className="text-sm text-gray-400">Coach is typing…</div>
        )}
        <div ref={bottomRef} />
      </div>
