...
export class IntervalsClient {
  ...
  async getWellness(start: string, end: string): Promise<ICUWellness[]> {
    ...
    return raw.map(w => ({
      ...
      body_battery_low: ((w.bodyBatteryMin ?? w.bodyBatteryLow ?? w.body_battery_low) ?? null) as number | null,
      body_battery_high: ((w.bodyBatteryMax ?? w.bodyBatteryHigh ?? w.body_battery_high) ?? null) as number | null,
