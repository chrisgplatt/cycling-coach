...
`computeStrainTarget` is a pure function of a single `recoveryScore` input, and both consumers call it identically — but the Recovery score itself is computed independently at the two call sites: client-side in `app/dashboard/page.tsx` (from cached sync data) and server-side in `app/api/briefing/today/route.ts` (from a separate set of Supabase queries). This dual-computation of Recovery predates this feature and already carried a divergence risk before the target range existed; this feature makes that risk more visible by surfacing the *derived* range in two user-facing places (the dashboard ring and the AI coach's prose) rather than just the Recovery number itself. Unifying Recovery-score sourcing into one shared source of truth would close this gap, but is a separate, larger project (analogous in scope to the original Strain formula unification) — not addressed here.
