...
- `PATCH /api/ftp/[id]/apply` failure: the modal stays open with an error banner so the athlete can retry. The two writes inside the handler are ordered — `ftp_predictions.confirmed` is set to `true` first, and the `user_profile.current_ftp` update is only attempted if that first write succeeds — so a failure at the *first* write leaves the prediction `confirmed: false` and never touches the profile, which is the clean case described above. However, ordering does not make the handler atomic: a failure at the *second* write (the profile update) leaves the prediction row marked `confirmed: true` even though `user_profile.current_ftp` was never changed, i.e. a real partial/inconsistent state — the prediction reads as "applied to profile" when it wasn't. This is an accepted tradeoff, not a bug: fixing it would require a cross-table transaction (a new Supabase RPC/migration), which is out of scope for this feature. After the profile update succeeds, the handler also pushes the new FTP to intervals.icu via `syncFtpToIntervalsIcu`; that call is best-effort — its failures are swallowed internally and never roll back or block the response, so the apply is still reported as successful to the client even if the ICU push didn't land.
