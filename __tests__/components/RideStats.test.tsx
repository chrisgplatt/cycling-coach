...
const metrics: ActivityMetrics = {
  ...
  lr_balance: 52, best_efforts: [
    { secs: 5, watts: 900 }, { secs: 15, watts: 650 }, { secs: 30, watts: 500 }, { secs: 60, watts: 380 },
...
describe('RideStats adapters', () => {
  it('maps an ICUActivity', () => {
    ...
    expect(d).toMatchObject({
      ...
      best: { p5s: null, p15s: null, p30s: null, p1: 380, p5: 320, p10: 300, p20: 280, p60min: null },
    })
  })

  it('maps an ICUActivity, filling 5s/15s/30s/60min from a supplied best_efforts list', () => {
    const d = rideStatsFromActivity(activity, [{ secs: 5, watts: 900 }, { secs: 15, watts: 650 }, { secs: 30, watts: 500 }, { secs: 3600, watts: 220 }])
    expect(d.best).toMatchObject({ p5s: 900, p15s: 650, p30s: 500, p1: 380, p5: 320, p10: 300, p20: 280, p60min: 220 })
  })
  ...
  it('maps ActivityMetrics, looking up best efforts by secs and tolerating gaps', () => {
    ...
    expect(d).toMatchObject({
      avgWatts: 200, np: 210, tss: 85, distanceM: 30000, elevationM: 320,
      durationSecs: 3600, avgHr: 145, lrBalanceRight: 52,
      best: { p5s: 900, p15s: 650, p30s: 500, p1: 380, p5: 320, p10: null, p20: 280, p60min: 220 }, // 600s effort absent → null
    })
  })
})
...
describe('RideStats render', () => {
  ...
  it('shows 5s, 15s, 30s and 60min best power cells alongside 1/5/10/20 min', () => {
    const d = rideStatsFromMetrics(metrics, 3600, 85)
    render(<RideStats data={d} />)
  ...
  it('shows the Best Power card with placeholders when only some durations are present', () => {
    const d = rideStatsFromActivity(activity) // p5s/p15s/p30s/p60min null, p1/p5/p10/p20 present
    render(<RideStats data={d} />)
