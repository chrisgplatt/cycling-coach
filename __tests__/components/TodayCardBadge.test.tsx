...
describe('recovery score breakdown modal', () => {
  const wellness = {
    id: '2026-06-30',
    ctl: 60, atl: 65, form: -5, hrv: 52, resting_hr: 58,
    sleep_secs: 28800, body_battery_low: 30, body_battery_high: 85,
  ...
  it('opens the breakdown modal when the Recovery chip is tapped', () => {
    render(<TodayCard workout={null} wellness={wellness} hrvBaseline={50} />)
    expect(screen.queryByText('Recovery Breakdown')).not.toBeInTheDocument()
    fireEvent.click(screen.getByRole('button', { name: /recovery score breakdown/i }))
  ...
  it('closes the breakdown modal via the Close button', () => {
    render(<TodayCard workout={null} wellness={wellness} hrvBaseline={50} />)
    fireEvent.click(screen.getByRole('button', { name: /recovery score breakdown/i }))
    expect(screen.getByText('Recovery Breakdown')).toBeInTheDocument()
  ...
  it('does not close the breakdown modal on backdrop click', () => {
    render(<TodayCard workout={null} wellness={wellness} hrvBaseline={50} />)
    fireEvent.click(screen.getByRole('button', { name: /recovery score breakdown/i }))
  ...
  it('shows "Not available" for components with no data', () => {
    const minimalWellness = {
      id: '2026-06-30',
      ctl: null, atl: null, form: null, hrv: null, resting_hr: null,
      sleep_secs: null, body_battery_low: null, body_battery_high: null,
      stress_avg: null, stress_high: null, garmin_training_load: null, sleep_score: null,
    }
    render(<TodayCard workout={null} wellness={minimalWellness} />)
    fireEvent.click(screen.getByRole('button', { name: /recovery score breakdown/i }))
